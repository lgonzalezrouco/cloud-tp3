...
resource "null_resource" "build_frontend" {
  ...
  provisioner "local-exec" {
    ...
    command     = substr(pathexpand("~"), 0, 1) == "/" ? "bash build_frontend.sh ${module.backend_load_balancer.dns_name}/api https://${module.s3_images_bucket.bucket_id}.s3.${var.aws_region}.amazonaws.com ${aws_cognito_user_pool.matchmarket_user_pool.id} ${aws_cognito_user_pool_client.matchmarket_spa_client.id}" : "build_frontend.bat ${module.backend_load_balancer.dns_name}/api https://${module.s3_images_bucket.bucket_id}.s3.${var.aws_region}.amazonaws.com ${aws_cognito_user_pool.matchmarket_user_pool.id} ${aws_cognito_user_pool_client.matchmarket_spa_client.id}"
